...
# st.header('내용')은 title보다 한단계 작은 큰 제목(h2느낌)
st.header('안녕하세여!🐻‍❄️ Streamlit으로 만든 첫 페이지입니다')

# st.subheader('내용') header보다 한단계 작은 큰 제목(h3느낌)
st.subheader('오늘 배운 것 : 텍스트를 화면에 예쁘게 보여주는 방법;-)')

# st.text('내용') 꾸밈이 전혀 없는 순수 텍스트를 그대로 출력
st.text('st.text 로 출력한 문장입니다, 줄을 바꾸거나 굵게 등이 서식이 적용되지 않는다')

# st.caption('내용') 아주 작은 글씨로 보조 설명을 넣을 때
st.caption('이 문장은 st.ccaption으로 작성한 작은 보조 설명')

# st.markdown('') """ 마크다운 문법 """ 굵게,기울림,링크,목록(마크다운으로가능)
# st.markdown('----')
st.markdown(
    """
    ### 🐙마크다운으로 작성한 자기소개
