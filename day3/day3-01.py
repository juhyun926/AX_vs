"""
타이타닉 데이터셋 기초 탐색
pandas head/tail/shape/info/columns를 사용해서 데이터셋의기본 정보를
화면에 순서대로 보여주는 streamlit 앱이다.
실행방법 streamlit run day3-01.py
"""
import os
import io
import pandas as pd
...
if df is not None : 
    ...
    #io.StringIO()라는 '메모리 위에 가짜파일'에 결과를 받아낸 뒤 그 내용을 text로 보여준다
    ...
    st.success('기초 정보 확인 끝났습니다. 다음 예제에서 전처리 필터링 하께요🏝️')
