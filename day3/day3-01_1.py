...
실행방법 : streamlit run day3-01_1.py
"""
...
try:
    ...
    st.error('❌Titanic.csv 파일을 찾을수 없습니다🙅‍♀️')
...
# 성별 여자 남자 필터링 2컬럼 사용
    ...
    with col1:
        st.metric('여성 승객수', f'{len(female_df)}명')
    with col2:
        st.metric('남성 승객수', f'{len(male_df)}명')
        
        st.markdown('---')

        #두 조건을 동시에 만족하는 행(35세 이상 여성)

    st.subheader('3) 35세 이상 & 여성승객')
    over_35_female = df[(df['Age']>=35) & (df['Sex']== 'female')]
    st.write(f'35세 이상 & 여성승객 수: **{len(over_35_female)}명**')

st.markdown('---')

        #결측치(NaN) 확인 및 dropna처리
st.subheader('4) Age결측치 처리')
